...
- [x] Non-root user `boss` (UID 1000) with read-only-compatible layout
...
- Create non-root user `boss` (UID 1000, GID 1000, home `/home/boss`)
- Set `ENTRYPOINT ["/usr/bin/tini", "--"]` and `CMD ["bash"]`
- Set `USER boss` and `WORKDIR /home/boss`
...
**Agent name mapping** — [DR-002](../decisions/002-iteron-cli-commands.md#workspace-model) defines reserved agent names for `boss open` and tmux session naming. These map to binary commands as follows:
...
`boss open myproject claude` resolves to binary `claude` and creates tmux session `claude@myproject`. Verify exact binary names during implementation — upstream projects may change them.
...
- Read-only-compatible root: no agent writes outside `/tmp` and `/home/boss`
- Remove SUID/SGID binaries in image (`RUN find / -perm /6000 -exec chmod a-s {} +`)
- Writable paths at runtime: `/tmp` (tmpfs), `/home/boss` (volume mount)
...
| 9 | `podman run --rm <image> id` | `uid=1000(boss) gid=1000(boss)` |
...
| 12 | `podman run --rm <image> cat /home/boss/.claude.json` | JSON containing `"hasCompletedOnboarding": true` |
| 13 | `podman run --rm <image> cat /home/boss/.claude/settings.json` | JSON with tool-allow-all permission config |

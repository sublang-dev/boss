...
Validate that the local sandbox passes security hardening checks, has no unfixed critical/high CVEs (accepted CVEs with no applicable fix are tracked in `image/.trivyignore`), and is fully documented for end users.
...
- Capabilities dropped: `podman inspect boss-sandbox --format '{{.HostConfig.CapDrop}}'` → contains `ALL`
- Read-only root: `podman inspect boss-sandbox --format '{{.HostConfig.ReadonlyRootfs}}'` → `true`
- No-new-privileges: `podman inspect boss-sandbox --format '{{.HostConfig.SecurityOpt}}'` → contains `no-new-privileges`
- Write outside allowed paths: `podman exec boss-sandbox touch /usr/local/test` → exit 1, `Read-only file system`
- Write to allowed paths: `podman exec boss-sandbox touch /tmp/test && podman exec boss-sandbox touch /home/boss/test` → both exit 0

Already covered by integration tests in `tests/integration/start-stop.test.ts` (IR-002 tests 4–6): cap-drop ALL, read-only rootfs, no-new-privileges. Rootless mode is a host Podman property verified by `boss init`.
...
- Create `~/.local/bin` in the Dockerfile, owned by `boss:boss`
...
| 2 | `podman inspect boss-sandbox --format '{{.HostConfig.CapDrop}}'` | Contains `ALL` |
| 3 | `podman inspect boss-sandbox --format '{{.HostConfig.ReadonlyRootfs}}'` | `true` |
| 4 | `podman inspect boss-sandbox --format '{{.HostConfig.SecurityOpt}}'` | Contains `no-new-privileges` |
